#!/usr/bin/env bash
# Script: packages/server/scripts/06_fix_budget_tests.sh
# Objetivo: Corrigir testes de orçamento para não exceder limite de payload
# Uso: bash packages/server/scripts/06_fix_budget_tests.sh

set -euo pipefail

echo "[info] Corrigindo testes de orçamento (402)"

TARGET_FILE="packages/server/test/analyze-500.spec.ts"
...
# Reescrever arquivo com testes corrigidos
cat > "${TARGET_FILE}" << 'TESTEOF'
/**
...
echo "  - Testes de orçamento agora usam recordUsage(5000000)"
echo "  - Evita envio de payloads enormes que excedem limite do Fastify"
echo "  - Simula estado de orçamento excedido de forma realista"

# Executar testes para validar
echo ""
echo "[info] Validando correções com testes..."
