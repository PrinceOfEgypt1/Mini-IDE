#!/usr/bin/env bash
################################################################################
# Script: 26_fix_explore_timeline_last_test.sh
# Objetivo: Corrigir último teste falhando (busca de elemento <time>)
#
# Problema:
# - Teste busca <time> com getAllByRole('time'), mas <time> não tem role ARIA
# - HTML está correto, teste precisa usar querySelectorAll
#
# Solução:
# - Ajustar teste para buscar por tag name ao invés de role
################################################################################

set -euo pipefail

echo "[info] Corrigindo último teste do ExploreTimeline..."

TEST_FILE="packages/ui/src/components/explore/ExploreTimeline.test.tsx"

# Fazer backup
cp "$TEST_FILE" "${TEST_FILE}.backup-before-final-fix"

# Aplicar correção cirúrgica no teste de acessibilidade
cat > "$TEST_FILE" << 'EOF'
...
describe('ExploreTimeline', () => {
  ...
  describe('Acessibilidade', () => {
    it('deve ter estrutura semântica adequada', () => {
      ...
      // Elementos <time> HTML5 semânticos
      const timeElements = container.querySelectorAll('time');
      expect(timeElements.length).toBeGreaterThan(0);

      // Verificar atributo datetime
...
echo "[ok] Teste corrigido (busca por tag <time> ao invés de role)"
...
echo "  ✓ Teste de acessibilidade usa container.querySelectorAll('time')"
echo "  ✓ Valida atributo datetime em cada elemento <time>"
echo ""
echo "Resultado esperado: 13/13 testes passando ✓"
echo ""
echo "Próximo passo:"
