#!/usr/bin/env bash
# scripts/02-fix-index-ts.sh
#
# Descrição: Corrige packages/server/src/index.ts para ser módulo ESM válido
# Uso: bash scripts/02-fix-index-ts.sh
# Pré-requisitos: bash
# Efeitos colaterais: Sobrescreve packages/server/src/index.ts
...
echo "=== CORREÇÃO - packages/server/src/index.ts ==="
echo ""
...
if [[ -f "${TARGET_FILE}" ]]; then
  ...
  echo "[ok] Backup criado: ${TARGET_FILE}.bak"
fi

# Criar novo index.ts corrigido
...
echo ""
echo "Próximo passo: bash scripts/03-verify-typecheck.sh"
