#!/usr/bin/env bash
# scripts/11-fix-eslint.sh
#
# Descrição: Corrige erros de ESLint no index.ts
# Uso: bash scripts/11-fix-eslint.sh
# Pré-requisitos: bash
# Efeitos colaterais: Corrige index.ts
...
echo "========================================="
echo "CORREÇÃO - ESLint (index.ts)"
echo "========================================="
echo ""
echo "[info] Corrigindo ${TARGET_FILE}..."

# Backup
cp "${TARGET_FILE}" "${TARGET_FILE}.bak-eslint"

# Criar index.ts com correções de ESLint
...
echo "[ok] Arquivo corrigido"
echo ""

# Rodar lint
echo "[info] Running lint..."
if pnpm --filter @mini-ide/server lint; then
  echo "[ok] Lint passed!"
else
  echo "[erro] Lint failed"
  exit 1
fi
...
echo "  ✓ import type para FastifyRequest, FastifyReply"
echo "  ✓ FastifyInstance explícito no tipo de retorno"
echo "  ✓ Removido async dos handlers (não usam await)"
echo "  ✓ Tipos explícitos resolvem unsafe-call/member-access"
echo ""
