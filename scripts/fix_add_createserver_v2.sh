...
echo "[info] Corrigindo código do servidor"
...
export function registerRoutes(app: FastifyInstance): FastifyInstance {
  // GET /healthz - sem async pois não usa await
  app.get("/healthz", () => {
  ...
  app.post<{ Body: AnalyzeRequest }>("/analyze", async (request, reply) => {
    ...
    if (
      typeof effectiveMaxLen !== "number" ||
      effectiveMaxLen < MIN_MAX_LEN ||
      effectiveMaxLen > MAX_LEN_LIMIT
    ) {
      return reply.code(400).send({ error: "Bad Request" });
    }
...
// Executar com catch explícito
if (import.meta.url === `file://${process.argv[1]}`) {
  void main();
}
EOF

echo "[ok] Código corrigido"
...
echo "  1. Commitar: git add packages/server/ && git commit -m 'fix(server): corrigir após merge' && git push"
echo "  2. Checklist: REQUIRE_GLOBAL_CLI=0 bash ./42_pipeline_checklist.sh"
echo ""
